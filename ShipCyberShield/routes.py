# routes.py 상단에 필요한 import 문 추가
from flask import Blueprint, render_template, redirect, url_for, flash, request, jsonify, current_app, send_file
from flask_login import login_required, current_user
from sqlalchemy import desc
from app import app, db # app 임포트 확인
from models import Vessel, CBSAsset, SecurityZone, Alert, SecurityLog, SensorData, SyslogEntry, AlertThreshold, NotificationSetting, AlertStatus, AlertSeverity, UserRole, EventType # CBSAsset 임포트 확인
...
import netifaces
import logging
from simplified_scanner import scan_network, get_or_create_default_security_zone # 필요한 경우 simplified_scanner 사용

# Create a simple API blueprint
api_bp = Blueprint('api', __name__, url_prefix='/api')

@api_bp.route('/assets/<int:asset_id>/details', methods=['GET'])
@login_required
def get_asset_details_api(asset_id):
    asset = CBSAsset.query.get(asset_id)
    if not asset:
        logging.warning(f"Asset details requested for non-existent asset ID: {asset_id}")
        return jsonify({'error': 'Asset not found'}), 404

    # assets.html의 displayAssetDetails JavaScript 함수가 기대하는 형식으로 데이터 구성
    details = {
        'id': asset.id,
        'name': asset.name or "N/A",
        'ip_address': asset.ip_address or "N/A",
        'asset_type': asset.asset_type or "N/A",
        'category': getattr(asset, 'category', 'hardware'), # 'category' 필드가 모델에 없다면 기본값 설정
        'status': asset.status or 'unknown',
        'detection_method': getattr(asset, 'detection_method', 'Database Entry'),
        'hardware': {
            'name': asset.name or "N/A",
            'brand': asset.manufacturer or "N/A",
            'model': asset.model or "N/A",
            'main_function': asset.function or "N/A",
            'serial_number': asset.serial_number or "N/A",
            'physical_interfaces': asset.interfaces or "N/A",
            'system_software': asset.os_type or "N/A",
            'software_version': asset.os_version or "N/A", # OS 버전을 software_version으로 우선 매핑
            'firmware_version': asset.firmware_version or "N/A",
            'protocols': asset.protocols or "N/A"
        },
        'software': { # 소프트웨어 관련 정보는 모델에 따라 채워야 함
            'hardware_components': asset.name if asset.asset_type == 'Software' else 'N/A', # 예시
            'brand': asset.manufacturer if asset.asset_type == 'Software' else 'N/A',
            'model': asset.model if asset.asset_type == 'Software' else 'N/A',
            'main_function': asset.function if asset.asset_type == 'Software' else 'N/A',
            'version': asset.software_version or "N/A", # CBSAsset의 software_version 필드 사용
            'installed_date': asset.installation_date.isoformat() if asset.installation_date else 'N/A',
            'patch_level': asset.patch_level or "N/A",
        }
    }
    logging.info(f"Returning details for asset ID: {asset_id}")
    return jsonify(details)

@api_bp.route('/scan_device/<int:asset_id>', methods=['POST'])
@login_required
def scan_device_api(asset_id):
    asset = CBSAsset.query.get(asset_id)
    if not asset:
        logging.warning(f"Scan requested for non-existent asset ID: {asset_id}")
        return jsonify({'success': False, 'error': 'Asset not found'}), 404
    if not asset.ip_address:
        logging.warning(f"Scan requested for asset ID: {asset_id} with no IP address")
        return jsonify({'success': False, 'error': 'Asset has no IP address for scanning'}), 400

    scan_params = request.json
    logging.info(f"API call to scan device ID {asset_id} ({asset.ip_address}) with params: {scan_params}")

    # 여기에 실제 장비 스캔 로직을 구현해야 합니다.
    # 예시: simplified_scanner.ping_host, simplified_scanner.port_scan 또는 snmp_utils 사용
    # 지금은 시뮬레이션된 응답을 반환합니다.
    try:
        # (시뮬레이션) 실제 스캔 로직 호출 부분
        # from simplified_scanner import get_hostname, identify_device_type, port_scan # 예시
        # hostname = get_hostname(asset.ip_address)
        # open_ports_data = port_scan(asset.ip_address) # 기본 포트 스캔
        # device_type = identify_device_type(open_ports_data)
        # is_online = simplified_scanner.ping_host(asset.ip_address) # simplified_scanner에 ping_host가 있다고 가정

        # assets.html의 displayAssetDetails JavaScript 함수가 기대하는 형식으로 데이터 구성
        scanned_details = {
            'id': asset.id,
            'name': asset.name or f"Scanned-{asset.ip_address}", # 스캔 결과로 업데이트된 이름
            'ip_address': asset.ip_address,
            'asset_type': asset.asset_type, # 스캔으로 실제 타입이 파악되면 업데이트
            'category': getattr(asset, 'category', 'hardware'),
            'status': 'online', # 스캔 성공 시 'online'으로 가정
            'detection_method': 'Live Scan via API',
            'hardware': {
                'name': asset.name or f"Scanned-{asset.ip_address}",
                'brand': asset.manufacturer or "Scanned Brand",
                'model': asset.model or "Scanned Model",
                'main_function': asset.function,
                'serial_number': asset.serial_number or "SN-SCAN-" + str(asset.id),
                'physical_interfaces': asset.interfaces or "eth0 (scanned)",
                'system_software': asset.os_type or "Scanned OS",
                'software_version': asset.os_version or "1.0-scan",
                'firmware_version': asset.firmware_version or "FW-SCAN-1.0",
                'protocols': asset.protocols or "TCP, UDP (scanned)"
            },
            'software': {
                 'hardware_components': asset.name if asset.asset_type == 'Software' else 'N/A',
                 'brand': asset.manufacturer if asset.asset_type == 'Software' else 'N/A',
                 'model': asset.model if asset.asset_type == 'Software' else 'N/A',
                 'main_function': asset.function if asset.asset_type == 'Software' else 'N/A',
                 'version': asset.software_version or "N/A",
                 'installed_date': asset.installation_date.isoformat() if asset.installation_date else 'N/A',
                 'patch_level': asset.patch_level or "N/A",
            }
        }
        
        # 스캔 후 자산 정보 업데이트 (예: 상태, 마지막 스캔 시간)
        asset.status = 'online'
        asset.last_scan = datetime.utcnow()
        # 스캔 결과에 따라 asset.name, asset.manufacturer 등을 업데이트 할 수도 있습니다.
        db.session.commit()
        logging.info(f"Device scan successful for asset ID: {asset_id}")
        return jsonify({'success': True, 'device_info': scanned_details})

    except Exception as e:
        logging.error(f"Error during API device scan for asset {asset_id}: {str(e)}")
        # 오류 발생 시에도 JSON 형식으로 응답
        return jsonify({'success': False, 'error': f'Scan failed: {str(e)}'}), 500

@api_bp.route('/security_zones', methods=['GET'])
@login_required
def get_security_zones():
    """API endpoint to get security zones for a specific vessel or all zones"""
    vessel_id = request.args.get('vessel_id', type=int)
    
    # vessel_id가 제공된 경우 해당 vessel의 보안 영역만 반환
    if vessel_id:
        zones = SecurityZone.query.filter_by(vessel_id=vessel_id).all()
    else:
        # vessel_id가 없으면 모든 보안 영역 반환
...
def network_scan():
    ...
    # 보안 영역 목록 가져오기
    security_zones = SecurityZone.query.all()
...
def add_to_inventory():
    ...
    try:
        ...
        if not (device_json and zone_id):
        ...
        query = CBSAsset.query.filter_by(
            ip_address=device["ip"],
            security_zone_id=zone_id
        )
