...
class SecurityZone(db.Model):
    ...
    # Relationships
    vessel = db.relationship('Vessel', back_populates='security_zones')
    assets = db.relationship('CBSAsset', back_populates='security_zone', cascade='all, delete-orphan')
    
    # Ensure unique zone names within a vessel
    __table_args__ = (UniqueConstraint('vessel_id', 'name', name='uix_zone_vessel_name'),)
    
    def __repr__(self):
        return f'<SecurityZone {self.name} (Vessel: {self.vessel_id})>'
...
class CBSAsset(db.Model):
    ...
    vessel_id = db.Column(db.Integer, db.ForeignKey('vessel.id'), nullable=False)
    security_zone_id = db.Column(db.Integer, db.ForeignKey('security_zone.id'), nullable=False)
    parent_asset_id = db.Column(db.Integer, db.ForeignKey('cbs_asset.id'), nullable=True)  # For software installed on hardware
    created_at = db.Column(db.DateTime, default=datetime.datetime.utcnow)
    updated_at = db.Column(db.DateTime, default=datetime.datetime.utcnow, onupdate=datetime.datetime.utcnow)
    
    # Relationships
    vessel = db.relationship('Vessel', back_populates='assets')
    ...
    def __repr__(self):
        return f'<CBSAsset {self.name} (Vessel: {self.vessel_id}, Zone: {self.security_zone_id})>'
...
class SensorData(db.Model):
    ...
    # Relationships
    vessel = db.relationship('Vessel', back_populates='sensor_data')
    cbs_asset = db.relationship('CBSAsset', back_populates='sensor_data')
    
    def __repr__(self):
        return f'<SensorData {self.sensor_type} (Asset: {self.cbs_id}, Value: {self.value})>'
...
class Alert(db.Model):
    ...
    # Relationships
    vessel = db.relationship('Vessel', back_populates='alerts')
    ...
    def __repr__(self):
        return f'<Alert {self.title} (Status: {self.status.value}, Severity: {self.severity.value})>'
...
class SecurityLog(db.Model):
    ...
    def __repr__(self):
        return f'<SecurityLog {self.event_type.value} at {self.timestamp}>'
...
class AlertThreshold(db.Model):
    ...
    def __repr__(self):
        return f'<AlertThreshold {self.name} {self.metric_type} {self.comparison} {self.threshold_value}>'
...
class SyslogEntry(db.Model):
    ...
    def __repr__(self):
        return f'<SyslogEntry {self.hostname} {self.timestamp}: {self.message[:50]}...>'
